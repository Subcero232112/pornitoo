...
document.addEventListener('DOMContentLoaded', () => {
    // Comprobar si Firebase se inicializó correctamente (establecido en el HTML)
    if (!window.firebaseApp && !document.querySelector('div[style*="Error Crítico"]')) {
         ...
         // Mostrar un error si es necesario, aunque el HTML ya debería mostrar uno si falló
    } else if (window.firebaseApp){
        console.log("DOM ready and Firebase App detected. Initializing app logic...");
        initializeAppLogic(); // Llama a la función principal que contiene toda la lógica
    } else {
        console.log("DOM ready but Firebase init failed (error should be visible). App logic will not run.");
    }
}); // <--- Asegúrate que este cierre esté presente

// --- Función Principal que Contiene Toda la Lógica ---
function initializeAppLogic() {
    // --- 0. Firebase Services (Obtener referencias) ---
    // Verifica que window.firebaseApp exista antes de usarlo
    if (!window.firebaseApp) {
        console.error("initializeAppLogic called but firebaseApp is not ready!");
    ...
    // Añadir img solo si userAuthButton existe
    if (userAuthButton) {
        userAuthButton.appendChild(userPhotoElement);
    }
    ...
    // --- 2. Translations ---
    const translations = { /* ... (Objeto de traducciones completo como en respuestas anteriores) ... */
        es: {
             page_title: "Pornitoo", index_page_title: "Inicio - Pornitoo", search_placeholder: "Buscar títulos...", search_button: "Buscar",
             genre_select: "Género", genre_action: "Acción", genre_comedy: "Comedia", login_tooltip: "Iniciar sesión", chat_tooltip: "Ir al Chat", settings_tooltip: "Configuración",
             popular_title: "Populares Ahora", loading: "Cargando...", settings_title: "Ajustes", settings_theme_title: "Tema de Color", settings_language_title: "Idioma", settings_other_title: "Otros Ajustes",
             settings_particles: "Efecto Partículas:", settings_more_soon: "(Más ajustes próximamente...)", login_modal_title: "Iniciar Sesión / Registro", login_email_label: "Correo Electrónico", login_password_label: "Contraseña",
             login_signup_button: "Registrarse", login_signin_button: "Iniciar Sesión", login_divider_or: "O", login_google_firebase: "Continuar con Google", login_modal_text_firebase: "Regístrate o inicia sesión para acceder a todas las funciones.",
             login_modal_text_chat: "Necesitas iniciar sesión para chatear.", back_button: "Volver", views_count: "Vistas", published_date: "Publicado:", related_videos_title: "Más Videos",
             description_title: "Descripción", comments_title: "Comentarios", logout_tooltip: "Cerrar sesión", chat_page_title: "Chat General", chat_loading: "Cargando mensajes...",
             chat_input_placeholder: "Escribe un mensaje...", no_related_videos: "No hay videos relacionados.", login_needed_for_chat: "Inicia sesión para chatear", login_needed_to_comment: "Inicia sesión para comentar",
             comment_placeholder: "Escribe tu comentario...", comment_send_button: "Enviar", "auth/invalid-email": "El formato del correo no es válido.", "auth/user-disabled": "Esta cuenta ha sido deshabilitada.",
             "auth/email-already-in-use": "Este correo ya está registrado.", "auth/weak-password": "La contraseña debe tener al menos 6 caracteres.", "auth/operation-not-allowed": "El inicio de sesión por correo/contraseña no está habilitado.",
             "auth/invalid-credential": "Credenciales inválidas.", "auth/missing-password": "Falta la contraseña.", "auth/network-request-failed": "Error de red. Revisa tu conexión.", "auth/too-many-requests": "Demasiados intentos fallidos. Intenta más tarde.",
             "auth/popup-closed-by-user": "Inicio de sesión cancelado.", "error_default": "Ocurrió un error inesperado."
         },
         en: {
             page_title: "Pornitoo", index_page_title: "Home - Pornitoo", search_placeholder: "Search titles...", search_button: "Search",
             genre_select: "Genre", genre_action: "Action", genre_comedy: "Comedy", login_tooltip: "Login", chat_tooltip: "Go to Chat", settings_tooltip: "Settings",
             popular_title: "Popular Now", loading: "Loading...", settings_title: "Settings", settings_theme_title: "Color Theme", settings_language_title: "Language", settings_other_title: "Other Settings",
             settings_particles: "Particle Effect:", settings_more_soon: "(More settings coming soon...)", login_modal_title: "Login / Sign Up", login_email_label: "Email Address", login_password_label: "Password",
             login_signup_button: "Sign Up", login_signin_button: "Sign In", login_divider_or: "OR", login_google_firebase: "Continue with Google", login_modal_text_firebase: "Sign up or log in to access all features.",
             login_modal_text_chat: "You need to be logged in to chat.", back_button: "Back", views_count: "Views", published_date: "Published:", related_videos_title: "More Videos",
             description_title: "Description", comments_title: "Comments", logout_tooltip: "Sign out", chat_page_title: "General Chat", chat_loading: "Loading messages...",
             chat_input_placeholder: "Type a message...", no_related_videos: "No related videos found.", login_needed_for_chat: "Log in to chat", login_needed_to_comment: "Log in to comment",
             comment_placeholder: "Write your comment...", comment_send_button: "Send", "auth/invalid-email": "Invalid email format.", "auth/user-disabled": "This account has been disabled.",
             "auth/email-already-in-use": "This email is already registered.", "auth/weak-password": "Password should be at least 6 characters.", "auth/operation-not-allowed": "Email/password sign-in is not enabled.",
             "auth/invalid-credential": "Invalid credentials.", "auth/missing-password": "Password is missing.", "auth/network-request-failed": "Network error. Check your connection.", "auth/too-many-requests": "Too many failed attempts. Try again later.",
    ...
    function formatDate(timestamp) { if (!timestamp) return '---'; try { const date = new Date(timestamp); return date.toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' }); } catch (e) { console.error("Error formatting date:", e); return 'Fecha inválida'; } }
    ...
    const authManager = (() => {
        ...
        function init() { onAuthStateChanged(auth, (user) => { console.log("Auth State Changed:", user ? `User Logged In (${user.uid})` : "User Logged Out"); const wasLoggedIn = !!currentUser; currentUser = user; updateAuthButtonUI(user); if (!wasLoggedIn || !user) { languageManager.setLanguage(languageManager.getCurrentLang()); } if (document.getElementById('detail-view-container')) { const detailVideoId = new URLSearchParams(window.location.search).get('id'); if (detailVideoId) setupCommentForm(detailVideoId); } }); if (userAuthButton) { userAuthButton.addEventListener('click', () => { if (currentUser) { logoutUser(); } else { hideLoginError(); uiManager.toggleModal($('#login-modal'), true); } }); } }
        function getCurrentUser() { return currentUser; }
        return { init, signInWithGoogle, signUpWithEmailPassword, signInWithEmailPassword, logoutUser, getCurrentUser };
    })();

    // --- 7.6. Función para Actualizar UI Botón/Sidebar ---
     function updateAuthButtonUI(user) {
         ...
         if (!sidebarPhoto || !sidebarName) { /*console.warn("Sidebar auth elements not found"); */} // Menos crítico si no estamos en chat.html
         ...
         if (user) { // Logged In
             ...
             const photoURL = user.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.displayName || user.email || 'U')}&background=random&color=fff&size=40`;
    ...
    // --- 8. Lógica Específica de Página (Firebase RTDB) ---
    const pageLogic = (() => {
            ...
            onValue(videosRef, (snapshot) => {
                ...
                if (snapshot.exists()) {
                    ...
                    Object.entries(videosData).forEach(([id, data]) => {
                        ...
                        posterItem.innerHTML = ` <div class="poster"> <img src="${data.posterUrl || defaultImgSrc}" alt="${data.title}" loading="lazy" onerror="this.onerror=null; this.src='${errorImgSrc}';"> </div> <h3 class="poster-title">${escapeHTML(data.title)}</h3> `; // Escape title
                        posterContainer.appendChild(posterItem);
                    });
                     themeManager.applyTheme(loadFromLocalStorage('selectedThemePornitoo', 'green'));
                } else { console.log("No videos found in database."); posterContainer.innerHTML = `<p style="grid-column: 1 / -1; text-align: center; color: var(--text-color-secondary);" data-translate-key="no_videos_available">No hay videos disponibles.</p>`; languageManager.setLanguage(languageManager.getCurrentLang()); }
        ...
        function initDetailPage() {
            ...
            onValue(videoRef, (snapshot) => {
                ...
                if (snapshot.exists()) {
                    ...
                    if (videoPlayerIframe) { videoPlayerIframe.src = data.videoUrl || ''; videoPlayerIframe.onload = () => { if(videoLoadingIndicator) videoLoadingIndicator.style.display = 'none'; }; videoPlayerIframe.onerror = () => { if(videoLoadingIndicator) videoLoadingIndicator.textContent = 'Error al cargar video'; }; } else { if(videoLoadingIndicator) videoLoadingIndicator.style.display = 'none'; }
                    ...
                    loadComments(itemId);
                    setupCommentForm(itemId); // Llama aquí DESPUÉS de cargar datos y saber si el user está logueado (via authManager)
    ...
    function displayComment(key, data) {
        ...
        // Insertar nuevos comentarios al principio
    ...
    function setupCommentForm(videoId) {
        ...
        const submitCommentButton = document.getElementById('submit-comment-button'); // Seleccionar botón una vez
        ...
        let currentSubmitButton = submitCommentButton;
        ...
        // Traducir elementos del form
        commentInput.placeholder = translations[currentLang]?.comment_placeholder || "Write your comment...";
        newSubmitButton.textContent = translations[currentLang]?.comment_send_button || "Send";
        ...
        if (user) {
            ...
            newSubmitButton.disabled = false; // Asegurar que esté habilitado

             newSubmitButton.addEventListener('click', async () => {
                 const commentText = commentInput.value.trim(); if (commentText === '') return;
                 // Usar serverTimestamp() que importamos
            ...
            newSubmitButton.disabled = true; // Deshabilitar botón si no está logueado

             let currentLoginLink = commentLoginLink;
             let newLoginLink = currentLoginLink.cloneNode(true); // Clonar para evitar listeners duplicados
    ...
    function startAppModules() {
        ...
        themeManager.init();
        languageManager.init(); // Prepara listeners y estado inicial
        particleManager.init();
        uiManager.init();
        authManager.init(); // Inicia escucha de Auth (esto llamará a setLanguage y setupCommentForm cuando cambie el estado)
        pageLogic.init();
    ...
    // Ya estamos dentro del DOMContentLoaded, así que no hay problema
    // initializeAppLogic(); // <-- Esta línea ya se llama arriba después de comprobar Firebase

} // Fin initializeAppLogic
// ¡Asegúrate de que no falte ningún cierre aquí abajo!

// No pongas código fuera del listener DOMContentLoaded o de initializeAppLogic si depende del DOM o Firebases
